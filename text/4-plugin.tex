...
a~vnější cyklus se může opakovat. 
    ...
    \STATE{pozice=ulož pozici ve vstupním souboru}
    \IF{existuje radek}
    \STATE{vnitřní cyklus, viz obr. \ref{fig:pseudokladnavzd-vnitrni}}
...
vzdáleností mezi body), s~tím rozdílem, že tentokráte již použité body vkládáme do seznamu hodnot.
    ...
    \STATE{x=1}
    \STATE{delkaposunu=absolutní hodnota uživatelem volené hodnoty posunu}
...
opakuje s~řádky o~jeden posunutými. 
    ...
    \centering
    \begin{algorithmic}[1]
    \STATE{posunovanyradek, radek1=přečti řádek}
    \STATE{pozice=ulož pozici ve vstupním souboru}
    \WHILE{existuje posunovanyradek}
    \STATE{seek(pozice)}
    \STATE{nasledujiciradek, radek2=přečti řádek}
    \STATE{pozice=ulož pozici ve vstupním souboru}
    \WHILE{x>0}
    ...
    \STATE{posunovanyradek, radek1=nasledujiciradek}
    \ENDWHILE
    \end{algorithmic}
