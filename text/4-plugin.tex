...
\begin{itemize}
	...
	sloupec by mohl obsahovat jakoukoli jinou, i~nečíselnou hodnotu). Zásuvný modul posune body
	i~bez těchto hodnot, ale v~primárním určení, měření radioaktivity, by bez této hodnoty postrádal smysl.
	
	\item RECS: Pod označením RECS se skrývá číslo bodu. Tato položka nezbytná není, avšak
	číslování bodů zajišťuje lepší čitelnost dat, pročež je silně doporučováno vždy. 
...
\begin{itemize}
	...
	\item position\_correction.py: Implementuje zásuvný modul do prostředí QGIS. Načítá jeho ikonu,
	...
	\item position\_correction\_dockwidget.py: Nejdůležitější ze základních souborů. Zde se přes volání
	\textit{.ui} souboru position\_correction\_dockwidget\_base.ui vytvořeného v~prostředí Qt Designer
	vyvolává grafické uživatelské rozhraní. Zajišťuje také funkčnost ve\-škerých jeho částí a~celkové
	dodržování interní maat, od načítání cest vstupních a~výstupních dat přes jejich zobrazování (volá
	show\_as\_layer.py) až po samotné volání posunu (move.py). Na pozadí dochází ke kontrole vstupujících údajů, v~případě nesrovnalostí
...
\textit{Lat\_deg} a~\textit{Lon\_deg}. 
...
Ve speciálním případě, kdy vstupuje do posunu o~hodnoty číslice~0, dochází k~pouhé\-mu kopírování vstupního
...
Posun o~konstantní vzdálenost je obsažen v~move.by\_distance. 
...
zájmu, pro potřeby elipsoidických operací tedy užíváme knihovnu prostředí QGIS QgsDistanceArea. Zde nastavíme
...
parametrů elipsoidu pro výpočet první geodetické úlohy (ta v~QgsDistanceArea implemetována
není, a~je třeba ji řešit v~samotném kódu). 
...
První geodetická úloha tedy probíhá až na tomto poli. 
...
Ježto by měl zásuvný modul fungovat i~při operacích, které kterakkolivěk postrádají důvod k~jeho využití, byl do jeho
vnitřností implementován i~posun o~nulovou vzdálenost. V~takovém případě se však vážně nejedná o~posun, nýbrž o~pouhé přepsání
...
důvodu byla využita funkce \textit{computeDistanceBearing} knihovny QgsDistanceArea.
...
analýzu, parametry elipsoidu a~vyvolání knihovny QgsDistanceArea. Též výpočet první geodetické
...
o~niž se bod posouvá. Nejdříve je zapotřebí funkcí \textit{computeDistanceBearing}
...
\textit{Gtm\_sec} a~jejich rozdíly. Tím dochází k~zesložitění kódu, ale rovněž k~potřebnému
...
probíhá s~pomocnou proměnnou (ze zvyku si ji označme \textit{x}), která představuje čas posunu od momentálního bodu. 
...
Rovnají-li se tyto dvě hodnoty, do paměti se uloží souřadnice bodu 1 (bodu, k~němuž se počítá trajektorie). 
